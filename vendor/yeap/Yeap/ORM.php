...
class ORM
{
	...
	{
		return self::objects(static::$key, 0, 0, $this->where, $limit, $offset, $order_by);
	}
	...
	public function find($id = NULL)
	{
		$func = 'fetch';
		if($id !== NULL) {
			$this->where(self::$key, $id);
			$this->limit(1);
			$func = 'row';
		}
		// Generate select statement SQL
		list($sql, $params) = static::$db->select(self::$column, $this->table, self::$where, self::$limit, self::$offset, self::$order_by);
		return static::$db->$func($sql, $params, NULL);
	}
	...
	public function field($field = NULL)
	{
		if($field) {
			...
			self::$column = implode(',', self::$column);
		}
		return $this;
	}
	...
	/**
	 * set where
	 * @usage where('id', 5) or where('id', '>', 5)
	 */
	public function where($field = NULL, $value1 = NULL, $value2 = NULL)
	{
		if($field && $value1 !== NULL) {
			if(is_array($field)) { // raw where
				foreach($field as $k => $v)
				{
					self::$where[$k] = $v;
				}
			} else if($value2 === NULL) {
				self::$where[$field] = $value1;
			} else {
				$i = static::$db->i;
				self::$where[] = "{$i}{$field}{$i} {$value1} {$value2}";
			}
		}
		return $this;
	}
	...
	{
		// Generate select statement SQL
		list($sql, $params) = static::$db->select(self::$column, $this->table, self::$where);
		return static::$db->column($sql, $params, 0);
	}
	...
	{
		$result = static::$db->update($this->table, $data, array(static::$key => $this->data[static::$key]));

		// Invalidate cache
		static::cache_delete($this->table . $this->data[static::$key]);
